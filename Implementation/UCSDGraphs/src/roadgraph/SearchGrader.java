...
/**
 * @author UCSD MOOC Development Team Grader for Module 2.
 */
public class SearchGrader implements Runnable {
	...
	public void run() {
		...
		try {
			runTest(1, "map1.txt", "Straight line (0->1->2->3->...)", new GeographicPoint(0, 0),
					new GeographicPoint(6, 6));

			runTest(2, "map2.txt", "Same as above (searching from 6 to 0)", new GeographicPoint(6, 6),
					new GeographicPoint(0, 0));

			runTest(3, "map3.txt", "Square graph - Each edge has 2 nodes", new GeographicPoint(0, 0),
					new GeographicPoint(1, 2));

			runTest(4, "ucsd.map", "UCSD MAP: Intersections around UCSD", new GeographicPoint(32.8756538, -117.2435715),
